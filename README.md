...
```mermaid
graph TD
    Client["客户端 (Web/App)"] -->|HTTP/REST| Controller["API 网关"]
    ...
    subgraph "核心服务"
        ...
        Service --> User["用户服务"]
        Service --> Diary["日记服务"]
        Service --> Assessment["评估服务"]
    end
    
    subgraph "AI & 检测引擎"
        Service -->|同步/异步| LLM_Agent["LLM 智能体"]
        LLM_Agent -->|调用| Ollama["Ollama 本地模型"]
        LLM_Agent -->|工具调用| WebSearch["联网搜索"]
        
        Service -->|消息投递| MQ["RabbitMQ"]
        MQ -->|消费| Detector["风险检测消费者"]
        Detector --> Rule["规则引擎"]
        Detector --> Emotion["情感分析"]
        Detector --> LLM_Check["LLM 语义分析"]
    end
